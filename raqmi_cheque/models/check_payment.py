...
class NormalPayments(models.Model):
    ...
    name = fields.Char(string="name", required=False, compute="get_title", readonly=True, store=True)
    ...
    def _total_amount(self):
        if self.payment_subtype:
            if self.amount == 0.0:
                raise exceptions.ValidationError(_("amount for checks must be more than zerO!"))
        else:
            if self.amount1 == 0.0:
                raise exceptions.ValidationError(_("amount for payment must be more than zerO!"))

    @api.onchange("partner_id")
    def get_partner_acc(self):
        for rec in self:
            if rec.partner_id.supplier_rank != 0:
                rec.account_id = rec.partner_id.property_account_payable_id.id
            elif rec.partner_id.customer_rank != 0:
    ...
    def get_title(self):
        for rec in self:
            if rec.partner_id:
                if (
                    rec.payment_method.payment_subtype == "issue_check"
                    and rec.payment_method
                ):
                    rec.name = "Cheques for Vendor " + str(rec.partner_id.name)
                if (
                    rec.payment_method.payment_subtype == "rece_check"
                    and rec.payment_method
                ):
                    rec.name = "Cheques from Customer " + str(rec.partner_id.name)
            else:
                rec.name = "*"
            return rec.name
    ...
    def action_confirm(self):
        ...
        move = {
            "name": "Partner Payment " + "Receipt:" + self.receipt_number,
    ...
    def print_timesheet(self, data):
        ...
        return self.env["report"].get_action(
            self, "raqmi_cheque.receipt_check_cash_payment", data=data
        )

    def unlink(self):
        if self.state == "posted":
            raise exceptions.ValidationError(_("You cannot delete posted payment"))
        else:
            return super(NormalPayments, self).unlink()
...
class PaymentsCheckCreate(models.Model):
    ...
    _sql_constraints = [('check_number_unique', 'unique(check_number)',
                         "The check number must be unique, this one is already assigned.")]

    @api.onchange("check_number", "bank")
    def _check_number(self):
        lst = self.env['native.payments.check.create'].search([])
        for number in lst:
            if self.check_number == number.check_number and number.id != self.id:
                if self.bank == number.bank and number.id != self.id:
                    raise exceptions.ValidationError(_('The check number is already assigned '
                                                       'by the same bank before, please enter another number'))

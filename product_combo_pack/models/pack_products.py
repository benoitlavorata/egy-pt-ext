...
class PackProducts(models.Model):
    ...
    product_tmpl_id = fields.Many2one('product.template', string=_('Product'))
    price = fields.Float('Price', compute='compute_price', store=True)
    quantity = fields.Integer('Quantity', default=1)
    qty_available = fields.Float(_('Quantity Available'), compute='compute_quantity_of_product',
                                 store=True, readonly=False)
    total_available_quantity = fields.Float(_('Total Quantity'))

...
class AccountAccount(models.Model):
    ...
    def get_account_type(self, qb_account_type):
        ...
        elif qb_account_type == 'OtherExpense':
            account_type = 'Expenses'
...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        ...
        if not payment_id.communication:
            payment_id.communication = 'Payment_'+str(payment_id.id)
        return payment_id
...
#     
    ...
    def _prepare_payment_dict(self,payment):
        ...
        if payment:
            ...
            vals.update({
                ...
                'payment_date': payment.get('payment_date') if payment.get('payment_date') else '',
            ...
            if 'communication' in payment and payment.get('communication'):
                vals.update({'communication':payment.get('communication')})
    ...
    def export_payments(self):
        ...
        if export_date:
            filters.append(('payment_date', '=', export_date))
    ...
    def get_paymet_dict(self,payment):
        ...
        if payment.communication:
            invoice_id = self.env['account.move'].search([('name','=',payment.communication)], limit=1)
        if invoice_id:
        ...
        payment_dict.update({
            ...
            'communication': payment.communication if payment.communication else '',

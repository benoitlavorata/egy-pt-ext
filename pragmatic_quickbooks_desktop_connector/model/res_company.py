...
class Qb_Config_Settings(models.Model):
    ...
    last_imported_qbd_id_for_account = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_payment_methods = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_partners = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_vendors = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_product = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_sale_orders = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_invoice = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_payments = fields.Char("Last Imported Modified Date")
    last_imported_qbd_id_for_purchase_orders = fields.Char("Last Imported Modified Date")
    ...
    import_acc_limit = fields.Char('Accounts', default=50)
    import_cus_limit = fields.Char('Customer', default=50)
    import_ven_limit = fields.Char('Vendors', default=50)
    import_pro_limit = fields.Char('Prdoucts', default=50)
    import_so_limit = fields.Char('Sales Orders', default=20)
    import_inv_limit = fields.Char('Invoice', default=20)
    import_pay_limit = fields.Char('Payments', default=50)
    import_po_limit = fields.Char('Purchase Order', default=20)

    export_updated_record = fields.Boolean('Export Updated Record')
    export_acc_limit = fields.Char('Accounts', default=50)
    export_cus_limit = fields.Char('Customer', default=50)
    export_ven_limit = fields.Char('Vendors', default=50)
    export_pro_limit = fields.Char('Prdoucts', default=50)
    export_so_limit = fields.Char('Sales Orders', default=20)
    export_inv_limit = fields.Char('Invoice', default=20)
    export_pay_limit = fields.Char('Payments', default=50)
    export_po_limit = fields.Char('Purchase Order', default=20)

    export_sales_order_date = fields.Date('Export Date', default=False)
    export_invoice_date = fields.Date('Export Date', default=False)
    export_payment_date = fields.Date('Export Date', default=False)
    export_purchase_order_date = fields.Date('Export Date', default=False)
    ...
    def onchange_qb_account_recievable(self):
        ...
        if model_id:
            field_id = self.env['ir.model.fields'].search(
                [('name', '=', 'property_account_receivable_id'), ('field_description', '=', 'Account Receivable'),
    ...
    def onchange_qb_account_payable(self):
        ...
        if model_id:
            field_id = self.env['ir.model.fields'].search(
                [('name', '=', 'property_account_payable_id'), ('field_description', '=', 'Account Payable'),
                 ('model_id', '=', model_id.id)])
            if field_id:
    ...
    def onchange_qb_income_account(self):
        ...
        if model_id:
            field_id = self.env['ir.model.fields'].search(
                [('name', '=', 'property_account_income_categ_id'), ('field_description', '=', 'Income Account'),
                 ('model_id', '=', model_id.id)])
            if field_id:
    ...
    def onchange_qb_expense_account(self):
        ...
        if model_id:
            field_id = self.env['ir.model.fields'].search(
                [('name', '=', 'property_account_expense_categ_id'), ('field_description', '=', 'Expense Account'),
                 ('model_id', '=', model_id.id)])
            if field_id:
...
#         print("\n\n test connection response",response.text)
    ...
    def check_tax_code_for_order_invoices(self):
        ...
        self.env.cr.execute("SELECT order_id FROM sale_order_line WHERE qbd_tax_code is NULL")
...
#                 print("\n\n im port account before response")
                try:
                    ...
                    raise Warning(e)
#                 print("\n\n im port account after response",type(response.text),response.text)
...
#                 print('Formated data [0 ] ',formatted_data)

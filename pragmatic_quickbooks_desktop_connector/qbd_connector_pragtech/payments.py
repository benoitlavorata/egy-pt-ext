...
def import_QBD_payments_to_TPA():
    try:
        ...
        for row in cursor.fetchall():
            ...
            odoo_payment_dict['payment_date'] = str(row_as_list[2])
    ...
    try:
        ...
        if 'payments_list' in req:
            ...
            for rec in req.get('payments_list'):
                ...
                payment_date = "{d'"+rec.get('payment_date')+"'}" 
                if rec.get('amount'):
                ...
                if ref_number:
                    check_record ="Select Top 1 TxnId,RefNumber from ReceivePayment where RefNumber='{}'".format(ref_number)

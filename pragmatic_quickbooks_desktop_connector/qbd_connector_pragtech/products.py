...
def import_QBD_Products_to_TPA():
    try:
        ...
        if to_execute_account == 1:
            products = "SELECT TOP {} ListId, Type, Description, Name, IsActive, SalesOrPurchaseDesc, SalesDesc, PurchaseDesc, ManufacturerPartNumber, PurchaseCost, SalesOrPurchasePrice, SalesPrice, QuantityOnHand, IncomeAccountRefListID, SalesTaxCodeRefFullName, SalesOrPurchaseAccountRefListID, IncomeAccountRefListID ,TimeModified FROM item where Type in ('ItemNonInventory','ItemInventory','ItemService') Order by TimeModified ASC".format(limit)
            cursor.execute(products)
        elif to_execute_account == 2:
            time_modified = "{ts'"+str(request.args.get('last_qbd_id'))+"'}" 
            # print (time_modified)
            products = "SELECT TOP {} ListId, Type, Description, Name, IsActive, SalesOrPurchaseDesc, SalesDesc, PurchaseDesc, ManufacturerPartNumber, PurchaseCost, SalesOrPurchasePrice, SalesPrice, QuantityOnHand, IncomeAccountRefListID, SalesTaxCodeRefFullName, SalesOrPurchaseAccountRefListID, IncomeAccountRefListID ,TimeModified FROM item where Type in ('ItemNonInventory','ItemInventory','ItemService') and timemodified >={}".format(limit, time_modified)
            cursor.execute(products)
...
# 17TimeModified
        ...
        for row in cursor.fetchall():
            ...
            if row_as_list[1] == 'ItemService':
                ...
                odoo_product_dict['default_code'] = row_as_list[3]
            ...
            if row_as_list[1] == 'ItemNonInventory':
                ...
                odoo_product_dict['description'] = row_as_list[2]
                odoo_product_dict['income_account_id'] = row_as_list[15]
                odoo_product_dict['barcode'] = row_as_list[8]
                odoo_product_dict['qty_available'] = 0.0
                odoo_product_dict['description_purchase'] = ''
                odoo_product_dict['last_time_modified'] = str(row_as_list[17])
